...
function inject_code_executescript(passedtabid, actionTask, resetUrl) {
  ...
  return new Promise(function (fulfill, reject) {
    ...
    var code;
    if (actionTask === 'play') {
      code = {code: "document.getElementsByClassName('ytp-play-button ytp-button')[0].click();"};
    } else if (actionTask === 'reset') {
      code = {code: "window.location.href = '" + resetUrl + "&feature=youtu.be&t=0s';"};
      console.log(code);
    } else {
      console.log('cannot recognize button');
    }
    try {
      chrome.tabs.executeScript(passedtabid, code
        , function (result) {
            console.log(result);
...
function generateYoutubePlayers(tabs) {
  ...
  for (var i = 0; i < tabs.length; i++) {
    ...
    document.getElementById("dynamicButtonReset" + i).addEventListener('click'
      , function(e){
          var gatherTab = e.target.name.split(" ")[0];
          var gatherURL = e.target.name.split(" ")[1];
          executeButton(parseInt(gatherTab), 'reset', gatherURL);
    });
    document.getElementById("dynamicButtonPlay" + i).addEventListener('click'
      , function(e){
          var gatherTab = e.target.name.split(" ")[0];
